namespace ActiveDirectoryHelper
{
	...
	public class ActiveDirectory
	{
		...
		/// user is not found.
		/// </returns>
		public ADUserInfo GetUserInfoBySamName(string samName )
		{
			if ( configuration.AllowForCaching )
			{
				var p = cacheFor_Users.FindBySamName( samName );
				if ( p != null )
				{
					Trace.TraceInformation(
						string.Format(
			...
			var entry = GetDirectoryEntry( ldapString );

			var searcher =
				new DirectorySearcher( entry )
				{
					Filter = ldapFilter,
					SizeLimit = adSizeReturn
				};

			// --

			var info = GetUserInfoBySearcher( searcher );
		...
		/// user is not found.
		/// </returns>
		public ADGroupInfo GetGroupInfoBySamName(string samName )
		{
			if ( configuration.AllowForCaching )
			{
				var p = cacheFor_Groups.FindBySamName( samName );
				if ( p != null )
				{
					Trace.TraceInformation(
						string.Format(
			...
			var info = GetGroupInfo( searcher );
		...
		/// user is not found.
		/// </returns>
		public ADUserInfo GetUserInfoByGuid(Guid guid )
		{
			if ( configuration.AllowForCaching )
			{
				var p = cacheFor_Users.FindByGuid( guid );
				if ( p != null )
				{
					Trace.TraceInformation(
						string.Format(
			...
			var guidString = HexEscape( guidBytes );
		...
		/// user is not found.
		/// </returns>
		public ADGroupInfo GetGroupInfoByGuid(Guid guid )
		{
			if ( configuration.AllowForCaching )
			{
				ADGroupInfo p =
		...
		public void PutUserInfo(ADUserInfo info )
		{
			try
			{
		...
		public void ChangePassword(ADUserInfo info, string oldPassword, string newPassword )
		{
			try
			{
				Impersonator impersonator = null;
		...
		/// </returns>
		public ADUserInfo GetUserInfoByUserNameAndPassword(string samName, string password )
		{
			Trace.TraceInformation(
				@"++++++++++++++++++++++++++++++++++++++++++++++++" );
			Trace.TraceInformation(
			...
			try
			{
				...
				else
				{
					...
					{
						throw new Exception(Resources.Str_ZetaLib_Core_Common_ActiveDirectory_01 );
					}
					else
					{
		...
		/// <summary>
		/// Get information about all groups in the Active Directory.
		/// </summary>
		/// <returns>
		/// Returns an array of information of all users.
		/// </returns>
			...
			bool enable )
		{
			...
			if ( result == null )
			{
				throw new ApplicationException(
					...
					Replace( @"Guid", userGuid.ToString() ) );
			}
			else
			{
				// --
			...
			DirectorySearcher searcher )
		{
			...
			else
			{
				...
				info.PrimaryGroupToken = ReadFieldInteger( GetRP( result.Properties[@"primaryGroupToken"] ) );
		...
		private static AuthenticationTypes AuthenticationType
		{
			get
			{
		...
		/// <param name="searcher">The searcher.</param>
		/// <returns></returns>
		private ADUserInfo GetUserInfoBySearcher(DirectorySearcher searcher )
		{
			searcher.SizeLimit = adSizeReturn;
			...
			// http://support.microsoft.com/?scid=kb%3Ben-us%3B321360
			...
			else
			{
				...
				// Add relative DN.
				string dn = result.Path;
				string basePath = string.Format( @"LDAP://{0}", configuration.LdapServer );
				if ( dn.StartsWith( basePath, StringComparison.InvariantCultureIgnoreCase ) )
				{
					dn = dn.Substring( basePath.Length ).Trim( '/' );
				}
		...
		private class ActiveDirectoryUserCache
		{
			...
			/// 
			/// </summary>
			private readonly List<ADUserInfo> cachedItems = new List<ADUserInfo>();
		...
		private class ActiveDirectoryGroupCache
		{
			...
			/// 
			/// </summary>
			private readonly List<ADGroupInfo> cachedItems = new List<ADGroupInfo>();

			#endregion
		}

		private readonly ActiveDirectoryUserCache cacheFor_Users;
		private readonly ActiveDirectoryGroupCache cacheFor_Groups;
