namespace ActiveDirectoryHelper.Helper
{
	...
	public delegate bool Equality<T>(
		T x,
		T y
		);
		...
		ICollection
	{
		...
		private class EqualityToInterfaceWrapper<TI> :
			IEqualityComparer<TI>
		{
			private readonly Equality<TI> _equality;

			public EqualityToInterfaceWrapper(
				Equality<TI> equality )
			{
				_equality = equality;
			}
		...
		{
			data = new Dictionary<T, Dummy>(
				new EqualityToInterfaceWrapper<T>( equality ) );
		}
		...
		{
			data = new Dictionary<T, Dummy>( comparer );
		}
		...
		public Set(
			...
			IEqualityComparer<T> comparer )
		{
			data = new Dictionary<T, Dummy>( capacity, comparer );
		}
			...
			IEqualityComparer<T> comparer )
		{
			...
			{
				data = new Dictionary<T, Dummy>( original.data, comparer );
			}
		}
		...
		public T[] ToArray()
		{
			var result = new T[data.Keys.Count];
